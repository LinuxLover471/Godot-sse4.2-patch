...
build() {
  ...
  _args=(
    -j$(nproc --all)
    cflags="$CFLAGS -O3 -DNDEBUG -march=core2 -msse3 -mssse3 -msse4.1 -fPIC -Wl,-z,relro,-z,now -w"
    cxxflags="$CXXFLAGS -O3 -DNDEBUG -march=core2 -msse3 -mssse3 -msse4.1 -fPIC -Wl,-z,relro,-z,now -w"
    linkflags="$LDFLAGS"
    ...
    builtin_zstd=no
    colored=yes
    debug_symbols=no # Turn off debug symbols.
    disable_exceptions=false
    platform=linuxbsd
    production=yes # Enable production build.
    pulseaudio=yes
    system_certs_path=/etc/ssl/certs/ca-certificates.crt
    target=editor
    use_llvm=no
    lto=full # Use full LTO for better performance.
    werror=no
  )
